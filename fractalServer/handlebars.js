module.exports = require('@frctl/handlebars')({
  helpers: {
    ...
    contains(needle, haystack, options) {
      return (haystack && haystack.indexOf(needle) > -1) ? options.fn(this):options.inverse(this)
    },
    ...
    getCookie: (root, options) => {
      if (root._env.request.headers.cookie) {
        ...
        if (cookies.length) {
          const cookie = cookies.find((c) => (c.split('=')[0]===options.hash.name ? c:false))
          const value = cookie ? cookie.substr(options.hash.name.length + 1):''
      ...
      return {
        key: 'nrl', logoVariants: ['badge.svg', 'text.svg', 'silhouette.svg'],
      }
    },
    ...
    isDev: (root, options) => {
      ...
      if (root._env.request.headers.host && root._env.request.headers.host.indexOf('localhost') > -1) {
